      ...
      <el-form label-width="80px" :inline="true">
          <el-form-item label="一级菜单">
            <el-select v-model="C1Id"  placeholder="请选择" @change="getSecond">
                <el-option v-for="(data) in selectFirst" :key="data.id" :label="data.name" :value="data.id">{{ data.name }}</el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="二级菜单">
            <el-select  v-model="C2Id" placeholder="请选择" @change="getThird">
               <el-option v-for="data in selectSecond" :key="data.id" :label="data.name" :value="data.id">{{ data.name }}</el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="三级菜单">
            <el-select v-model="C3Id" placeholder="请选择">
                <el-option v-for="data in selectThird" :key="data.id" :label="data.name" :value="data.id">{{ data.name }}</el-option>
            </el-select>
          </el-form-item>
      </el-form>
  </el-card>
